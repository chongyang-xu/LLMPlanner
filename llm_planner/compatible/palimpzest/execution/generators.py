# ported from palimpzest: src/palimpzest/query/generators/generators.py
...
from llm_planner.compatible.palimpzest.core.data.dataclasses import GenerationStats

from llm_planner.compatible.palimpzest.core.lib.fields import BytesField
from llm_planner.compatible.palimpzest.core.lib.fields import ImageBase64Field, ImageFilepathField, ImageURLField, ListField

from llm_planner.compatible.palimpzest.constants import PromptStrategy
from llm_planner.compatible.palimpzest.constants import Cardinality
from llm_planner.compatible.palimpzest.constants import Model
from llm_planner.compatible.palimpzest.constants import TOKENS_PER_CHARACTER
from llm_planner.compatible.palimpzest.constants import MODEL_CARDS

from llm_planner.compatible.palimpzest.utils.generation_helpers import get_json_from_answer

import llm_planner.compatible.palimpzest.prompts as prompts
...
class BaseGenerator(Generic[ContextType, InputType], ABC):
    ...
    def __init__(self, prompt_strategy: PromptStrategy = PromptStrategy.COT_BOOL):
        self.prompt_strategy = prompt_strategy
        self.cardinality = Cardinality.ONE_TO_ONE
        self.model = Model.GPT_4o
    ...
    def _generate_developer_prompt(self) -> str:
        # mark that only part of prompt_strategy is ported
        self._partially_ported_check()
...
class OpenAIGenerator(BaseGenerator[str | list[str], str]):
    ...
    def __init__(self, prompt_strategy: PromptStrategy,):
        super().__init__(prompt_strategy)

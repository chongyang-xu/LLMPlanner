...
async def main():
    ...
    while start_idx < len(nec_prompts_parts):
        curr_prompts = nec_prompts_parts[start_idx:min(start_idx +
                                                       80, len(nec_prompts_parts
                                                              ))]
    ...
    for i in range(len(sampled_dic['nec_prompts_instance'])):
        if i in idx_to_loop:
            ...
            nec_prompts_instances.extend([
                '' for i in range(len(sampled_dic['nec_prompts_instance'][i]))
            ])
    ...
    for i in range(len(sampled_dic['dep_prompts'])):
        ...
        for j in range(len(sampled_dic['dep_prompts'][i])):
            if (i, j) not in fin_idx_to_loop and (i,
                                                  'all') not in fin_idx_to_loop:
                ...
                temp_dep_content.append([
                    choice['message']['content']
                    for choice in dep_results_instances[curr_idx]['choices']
                ])

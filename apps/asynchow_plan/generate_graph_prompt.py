...
def generate_edge_list(edge_list: tuple, step_num: int):
    '''
    ...
    for ele in edge_list:
        ...
        if left_number == '0' or right_number == '0' or int(
                left_number) > step_num or int(right_number) > step_num:
            continue
...
def match_shuffled_edges(shuffled_deps, shuffled_to_orig):
...
def get_timedelta_interval(time_dic):
    ...
    for step, time_exp in time_dic.items():
        ...
        try:
            time_span = re.findall(
                r'\d+\s*(?:min|minute|minutes|hr|hour|hours|sec|second|seconds|week|weeks|day|days|month|months|year|years)',
                time_exp)[-1]
            time = re.findall(r'\d+', time_span)[-1]
            unit = re.findall(r'\b[a-z]+', time_span)[-1].strip()
            if unit in ['year', 'years', 'y']:
                delta = [
                    timedelta(days=int(time) * 365),
                    timedelta(days=int(time) * 366)
                ]
            elif unit in ['month', 'months', 'm']:
                # define a loose range for month
                # match other units in same format
                delta = [
                    timedelta(days=int(time) * 28),
                    timedelta(days=int(time) * 31)
                ]
...
def get_fin_time_dics(time_dics):
    ...
    for i in range(len(time_dics)):
        iter_timedelta_dic, stepwise_time_dic = get_timedelta_interval(
            time_dics[i])
        for key, val in iter_timedelta_dic.items():
            if key in temp_timedelta_dic and val:
                ...
                temp_timedelta_dic[key].append(
                    (val, len(temp_timedelta_dic.get(key, []))))
            elif val:
...
def match_shuffled_time(timedelta_dic, time_dic, orig_to_shuffled):
    '''
...
def find_plan_type(edge_list, step_num):
    ...
    if is_line_graph(edge_list):
        nodes_in_edge_list = set(
            [int(node) for edge in edge_list for node in edge])
        if nodes_in_edge_list == set([i + 1 for i in range(step_num)]):
            return 'sequential'
    return 'async'


def series_composition(edge_lists, nodes):
    '''
...
def merge_all_graphs(edge_list_instances, edge_list_part, step_nums_instances):
    '''
    ...
    for i, edge_list_instance in enumerate(temp_edge_list_instances):
        ...
        for j, edge in enumerate(edge_list_instance):
            temp_edge_list_instances[i][j] = (str(int(edge[0]) + plus_step),
                                              str(int(edge[1]) + plus_step))
    edge_list_dic = {
        str(i + 1): temp_edge_list_instances[i]
        for i in range(len(temp_edge_list_instances))
    }
    nodes = [[
        str(n)
        for n in range(start_idxs[i], start_idxs[i] + step_nums_instances[i])
    ]
             for i in range(len(step_nums_instances))]
    for edge in edge_list_part:
        edges = [edge_list_dic[edge[0]], edge_list_dic[edge[1]]]
        nodess = [nodes[int(edge[0]) - 1], nodes[int(edge[1]) - 1]]
        composition, updated_nodes = series_composition(edges, nodess)
    ...
    return fin_edge_list, generate_edge_list(tuple(fin_edge_list),
                                             total_step_num)
...
def get_all_dependencies_proscript(text, aux_idx):
    ...
    Get all dependencies from model outputs
        ...
        Returns:
            all_deps: set
    '''
    patterns = re.findall(r'(?:step)?\s*\d+"?\'?\s*->\s*"?\'?(?:step)?\s*\d+',
                          text.lower())
    all_deps = set()
    if not patterns:
        return all_deps
...
def sample_test_instances(full_benchmark, from_step, to_step, bin_width,
                          plan_type):
    sampled_idxs = {key: list() for key in range(from_step, to_step + 1)}
    sampled_benchmark = {
        key: list() for key in full_benchmark.keys() if key != 'prompts'
    }
    ...
    for key in sampled_idxs.keys():
        random.seed(2024)
        curr_idx = random.sample(sampled_idxs[key],
                                 min(bin_width, len(sampled_idxs[key])))
        for idx in curr_idx:
                ...
                if key != 'prompts':
                    ...
                    for template, data in full_benchmark[key].items():
                        ...
                        for key2, val in data.items():
                            ...
                            sampled_benchmark[key][template][key2].append(
                                val[idx])
...
def generate_prompt(
    ...
    if not step_deps:
        ...
        if not adjacency_list:
            for edge in step_deps:
                temp_constraint = ordering_template.replace(
                    '[preceding step]', f'step {edge[0]}')
                temp_constraint = temp_constraint.replace(
                    '[following step]', f'step {edge[1]}')
            ...
            for prec, foll in adjacency_list.items():
                ...
                foll_steps = ', '.join(
                    meaningful_folls[:-1]
                ) + ' and ' + meaningful_folls[-1] if len(
                    meaningful_folls) > 1 else meaningful_folls[0]
                temp_constraint = ordering_template.replace(
                    '[preceding step]', f'step {prec}')
                temp_constraint = temp_constraint.replace(
                    '[following step]', f'step {foll_steps}')
                ordering_constraints.append(f'{temp_constraint}\n'.capitalize())
        prompt = prompt.replace('[ORDERING CONSTRAINTS]',
                                ''.join(sorted(ordering_constraints)))

    if graph and graph_type:
        if graph_type == 'adjacency_list':
            prompt = prompt.replace(
                '[GRAPH]',
                f'Here is the adjacency list representation of the step ordering constraints:\n{graph}\nTime for each step can be represented as a dictionary:\n{time_dic}'
            )
        elif graph_type == 'adjacency_matrix':
            prompt = prompt.replace(
                '[GRAPH]',
                f'Here is the adjacency matrix representation of the step ordering constraints (assuming the node order to be 1 to {len(steps)}, \'END\', \'START\'):\n{graph}\nTime for each step can be represented as a dictionary:\n{time_dic}'
            )
        elif graph_type == 'csr':
            values, row_indices, col_indices = graph
            prompt = prompt.replace(
                '[GRAPH]',
                f'Here is the compressed sparse row representation of the step ordering constraints (assuming the node order to be 1 to {len(steps)}, \'END\', \'START\'):\nValues: {values}\nRow indices: {row_indices}\nColumn indices: {col_indices}\nTime for each step can be represented as a dictionary:\n{time_dic}'
            )
        elif graph_type == 'edge_list':
            prompt = prompt.replace(
                '[GRAPH]',
                f'Here is the edge list representation of the step ordering constraints:\n{graph}\nTime for each step can be represented as a dictionary:\n{time_dic}'
            )
        else:
    ...
    if bag:
        ...
        prompt = prompt.replace(
            'Answer the time in double quotes.',
            'Let\'s construct a graph with the nodes and edges first to represent step ordering constraints, and also construct a dictionary to represent time needed for each step. Use the graph and dictionary to calculate the shortest possible time needed for the task. Answer the time in double quotes.'
        )

    if cot:
        prompt = prompt.replace(
            'Answer the time in double quotes.',
            'Let\'s think step by step and then answer the time in double quotes.'
        )
...
def main():
    ...
    for i in range(len(sampled_dic['titles'])):
        if not sampled_dic['multi_parts'][i] or len(
                res_dic['consistent_instance'][i]) == 1:
            ...
            for ii, cons in enumerate(consistent_instances):
                if cons and cons >= 4:
                    shuffled_to_orig = match_indexes(
                        sampled_dic['shuffled_instances_steps'][i][ii],
                        sampled_dic['steps'][i][ii][-1])
                    # print(cons, shuffled_to_orig, res_dic['dep_results_instances'][i][ii])
                    orig_edge_list = match_shuffled_edges(
                        res_dic['most_freq_deps_instance'][i][ii],
                        shuffled_to_orig)
                    complete_edge_list = generate_edge_list(
                        tuple(orig_edge_list), step_nums[ii])
                    ...
                    ) = edge_list_to_adjacency_structures(complete_edge_list)
                    fin_timedelta_dic, fin_time_dic = get_fin_time_dics(
                        res_dic['time_dics'][i][ii])
                    all_paths, shortest_path_len, longest_path_len, path_count = get_all_paths(
                        adjacency_list, 'START', 'END')
                    all_corr_paths, task_time = generate_task_time(
                        all_paths, fin_timedelta_dic)
                    fin_res_dic['titles'].append(sampled_dic['titles'][i])
                    fin_res_dic['multi_methods'].append(
                        sampled_dic['multi_methods'][i])
                    fin_res_dic['multi_parts'].append(
                        sampled_dic['multi_parts'][i])
                    fin_res_dic['steps'].append(sampled_dic['steps'][i][ii][-1])
                    fin_res_dic['step_nums'].append(step_nums[ii])
                    fin_res_dic['sub_titles'].append(
                        sampled_dic['steps'][i][ii][0])
            ...
            if (4 not in consistent_instances) and (
                    5 not in consistent_instances):
                continue
            for ii, cons in enumerate(consistent_instances):
                if cons and cons >= 4:
                    edge_list_instances.append(
                        res_dic['most_freq_deps_instance'][i][ii])
                else:
            ...
            if not consistent_parts or consistent_parts < 4 or (
                [] in consistent_instances) or (1 in consistent_instances) or (
                    2 in consistent_instances) or (3 in consistent_instances):
                for ii, edge_list_instance in enumerate(edge_list_instances):
                    if edge_list_instance is None:
                        continue
                    if not consistent_instances[ii] or consistent_instances[
                            ii] < 4:
                        continue
                    shuffled_to_orig = match_indexes(
                        sampled_dic['shuffled_instances_steps'][i][ii],
                        sampled_dic['steps'][i][ii][-1])
                    orig_edge_list = match_shuffled_edges(
                        edge_list_instance, shuffled_to_orig)
                    complete_edge_list = generate_edge_list(
                        tuple(orig_edge_list), instance_step_nums[ii])
                    plan_type = find_plan_type(orig_edge_list,
                                               instance_step_nums[ii])
                    ve = len(complete_edge_list) + instance_step_nums[ii] + 2
                    adjacency_list, adjacency_matrix, (
                        values, row_indices, col_indices
                    ) = edge_list_to_adjacency_structures(complete_edge_list)
                    fin_timedelta_dic, fin_time_dic = get_fin_time_dics(
                        res_dic['time_dics'][i][0])
                    try:
                        all_paths, shortest_path_len, longest_path_len, path_count = get_all_paths(
                            adjacency_list, 'START', 'END')
                    except Exception:
                ...
                shuffled_to_orig_part = match_indexes(
                    sampled_dic['shuffled_part_steps'][i],
                    [step[0] for step in sampled_dic['steps'][i]])
                orig_edge_list_part = match_shuffled_edges(
                    edge_list_part, shuffled_to_orig_part)
                shuffled_to_orig_instances = [
                    match_indexes(
                        sampled_dic['shuffled_instances_steps'][i][ii],
                        sampled_dic['steps'][i][ii][-1]) for ii in range(
                            len(sampled_dic['shuffled_instances_steps'][i]))
                ]
                orig_edge_list_instances = [
                    match_shuffled_edges(edge_list_instances[ii],
                                         shuffled_to_orig_instances[ii])
                    for ii in range(len(edge_list_instances))
                ]
                all_edge_list, complete_edge_list = merge_all_graphs(
                    orig_edge_list_instances, orig_edge_list_part,
                    instance_step_nums)
                plan_type = find_plan_type(all_edge_list,
                                           sum(instance_step_nums))
                ...
                fin_res_dic['steps'].append([
                    step for steps in sampled_dic['steps'][i]
                    for step in steps[-1]
                ])
                fin_res_dic['step_nums'].append(sum(instance_step_nums))
                fin_res_dic['sub_titles'].append(
                    [steps[0] for steps in sampled_dic['steps'][i]])
                fin_res_dic['orig_idx'].append(sampled_dic['orig_idx'][i])
                fin_res_dic['edge_list'].append(complete_edge_list)
    ...
    for i in range(len(proscript)):
        ...
        temp_steps = [
            step.split(': ')[1] for step in proscript[i]
            ['flatten_output_for_script_generation'].split(';')[:step_num + 2]
        ]
        temp_steps[0] = 'NONE'
        try:
            shuffled_to_orig = match_indexes([
                proscript[i]['events'][str(s)]
                for s in range(len(proscript[i]['events']))
            ], temp_steps)
        except Exception:
            print('index match error', i)
            continue
        deps = '; '.join(
            proscript[i]['flatten_output_for_script_generation'].split(
                ';')[step_num + 2:])

        temp_time_dic = {
            str(shuffled_to_orig[int(key)]): val
            for key, val in proscript[i]['events_minutes'].items()
        }
        ...
        adjacency_list, adjacency_matrix, (
            values, row_indices,
            col_indices) = edge_list_to_adjacency_structures(edge_list)
        try:
            all_paths, shortest_path_len, longest_path_len, path_count = get_all_paths(
                adjacency_list, 'START', 'END')
        except Exception:
        ...
        formatted_proscript['steps'].append(temp_steps[1:aux_idx] +
                                            temp_steps[aux_idx + 1:])
    ...
    for dic in [fin_res_dic, formatted_proscript]:
        ...
        for ordering_template in nl_prompt_templates:
            dic['prompts'][ordering_template] = {
                'vanilla_prompts': list(),
            ...
            for i in range(len(dic['titles'])):
                for combined in ['', '_combined']:
                    if combined:
                        ...
                        adjacency_list = None
                    dic['prompts'][ordering_template][
                        f'vanilla{combined}_prompts'].append(
                            generate_prompt(
                                dic['titles'][i],
                                dic['steps'][i],
                                dic['time_dic'][i],
                    ...
                    dic['prompts'][ordering_template][
                        f'cot{combined}_prompts'].append(
                            generate_prompt(
                                dic['titles'][i],
                                dic['steps'][i],
                                dic['time_dic'][i],
                                ...
                                ordering_template=ordering_template))
                    dic['prompts'][ordering_template][
                        f'bag{combined}_prompts'].append(
                            generate_prompt(
                                dic['titles'][i],
                                dic['steps'][i],
                                dic['time_dic'][i],
                                ...
                                ordering_template=ordering_template))
                    dic['prompts'][ordering_template][
                        f'bag_cot{combined}_prompts'].append(
                            generate_prompt(
                                dic['titles'][i],
                                dic['steps'][i],
                                dic['time_dic'][i],
                                ...
                                ordering_template=ordering_template))
                    for graph_type in [
                            'edge_list', 'adjacency_list', 'adjacency_matrix',
                            'csr'
                    ]:
                        dic['prompts'][ordering_template][
                            f'{graph_type}{combined}_prompts'].append(
                                generate_prompt(
                                    dic['titles'][i],
                                    dic['steps'][i],
                                    dic['time_dic'][i],
                                    ...
                                    ordering_template=ordering_template))
                        dic['prompts'][ordering_template][
                            f'{graph_type}_cot{combined}_prompts'].append(
                                generate_prompt(
                                    dic['titles'][i],
                                    dic['steps'][i],
                                    dic['time_dic'][i],
                                    ...
                                    ordering_template=ordering_template))
    aligned_res_dic = {
        key: list() for key in fin_res_dic.keys() if key != 'prompts'
    }
    ...
    for i in range(len(fin_res_dic['titles'])):
        ...
        for ordering_template in fin_res_dic['prompts'].keys():
            ...
            for key, val in fin_res_dic['prompts'][ordering_template].items():
                ...
                aligned_res_dic['prompts'][ordering_template][key].append(
                    val[i])

    benchmark = {
        key: list() for key in formatted_proscript.keys() if key != 'prompts'
    }
            ...
            for template, data in formatted_proscript[key].items():
                ...
                for key2, val in data.items():
                    if key2 not in benchmark[key][template]:
                        benchmark[key][template][
                            key2] = val + aligned_res_dic[key][template][key2]
    ...
    seq_benchmark = sample_test_instances(benchmark, 3, 7, 40, 'sequential')
    para_benchmark = sample_test_instances(benchmark, 3, 7, 40, 'parallel')

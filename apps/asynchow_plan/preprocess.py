...
def read_file_to_dict(path, helpful_percent=60, load_by_method=True):
    '''
    ...
    '''
    wikihow_dic = {
        'titles': list(),  # list of titles for each plan
        'steps': list(),  # list of steps for each plan
        'multi_methods':
            list(),  # list of whether each plan has more than one method
        'multi_parts':
            list(),  # list of whether each plan has more than one part
        'step_num': list(),  # number of steps in each plan
        'max_step_num': list(),  # maximum number of steps in all plans
        'original_data': list(
        ),  # store the original data for error analysis but we should remove it in final submission for anonymity
        'helpful_percent': list(),  # helpfulness of each plan
        'n_votes':
            list()  # number of votes for each plan
    }
    ...
    for subpath in wikihow_path:
        ...
        # sanity check
        if (task.get('methods') and task.get('steps')) or (
                task.get('methods') and
                task.get('parts')) or (task.get('steps') and task.get('parts')):
            print(task)
            break
        if not task['rating']['helpful_percent'] or task['rating'][
        ...
        if task.get('steps'):
            ...
            for step in task['steps']:
                headline = replace_url(
                    step['headline']).strip() + ': ' + replace_url(
        ...
        if not steps:
            continue
        if tuple(sorted(steps[-1][-1])) not in non_dup_plans or (
                task.get('methods') and load_by_method) or task.get('parts'):
            non_dup_plans.add(tuple(sorted(steps[-1][-1])))
            wikihow_dic['titles'].append(title)
            wikihow_dic['steps'].append(steps)
            wikihow_dic['max_step_num'].append(max(step_num))
            wikihow_dic['helpful_percent'].append(
                task['rating']['helpful_percent'])
            ...
            if task.get('methods'):
                ...
                sorted_idx = sorted(range(len(s)),
                                    key=lambda k: s[k],
                                    reverse=True)
                wikihow_dic['step_num'][-1] = [s[i] for i in sorted_idx]
                wikihow_dic['steps'][-1] = [
                    wikihow_dic['steps'][-1][i] for i in sorted_idx
                ]
            else:
...
def generate_wikihow_task_description(wikihow_data, idx, task):
    ...
    assert not (wikihow_data['multi_parts'][idx] and
                wikihow_data['multi_methods'][idx])
    ...
    if task == 'dependency':
        if wikihow_data['multi_parts'][idx]:
            ...
            random.seed(0)
            part_steps = '\n'.join([
                f'step{ii+1}: {s}' for ii, s in enumerate(
                    random.sample([s[0] for s in wikihow_data['steps'][idx]],
                                  len(wikihow_data['steps'][idx])))
            ])
            ...
            for part in wikihow_data['steps'][idx]:
                ...
                steps = '\n'.join([
                    f'step{ii+1}: {s}' for ii, s in enumerate(
                        random.sample(part[-1], len(part[-1])))
                ])
                ind = len(part[-1])
                steps += f'\n{ind+1}: NONE.'
                steps += f'\n{ind+2}: {part_name}.'
            ...
            for method in wikihow_data['steps'][idx]:
                ...
                steps = '\n'.join([
                    f'step{ii+1}: {s}' for ii, s in enumerate(
                        random.sample(method[-1], len(method[-1])))
                ])
                ind = len(method[-1])
                steps += f'\n{ind+1}: NONE.'
                steps += f'\n{ind+2}: {method_name}.'
            ...
            steps = '\n'.join([
                f'step{ii+1}: {s}' for ii, s in enumerate(
                    random.sample(wikihow_data['steps'][idx][-1][-1],
                                  len(wikihow_data['steps'][idx][-1][-1])))
            ])
        ...
        if not wikihow_data['multi_methods'][idx]:
            ...
            steps = '\n'.join(
                [f'step{ii+1}: {s}' for ii, s in enumerate(step_data)])
...
def contain_keywords(steps):
    ...
    '''
    keywords = set([
        'this', 'above', 'below', 'keep', 'know', 'knowing', 'opt', 'if',
        'when', 'while', 'become', 'be', 'stay', 'repeat', 'after', 'before'
    ])
    words_in_steps = set(
        re.findall(r"[\w']+",
                   ' '.join([step.lower().split(': ')[0] for step in steps])))

...
def gen_proscript_dep_prompt(script):
    ...
    steps = [
        step.split(': ')[1] for i, step in enumerate(
            script['flatten_input_for_edge_prediction'].split('; '))
    ][:-2]
...
def get_gold_edges_proscript(script, step_num):
    '''
    ...
    for edge in script['gold_edges_for_prediction']:
        ...
        if int(left) in [step_num, step_num - 1
                        ] or int(right) in [step_num, step_num - 1]:
            continue
...
def eval_proscript(responses, proscript_test, idxs: list):
    total = 0
    recall = 0
    prec = 0
    f1 = 0
    ...
    for i, response in enumerate(responses):
        step_num = len(proscript_test[idxs[i]]
                       ['flatten_input_for_edge_prediction'].split('; ')) - 1
...
async def main():
    ...
    for seed in range(3):
        for key, val in proscript.items():
            ...
            while iter_idx < 100:
                prompts = [
                    gen_proscript_dep_prompt(val[sampled_idx[i]])
                    for i in range(iter_idx, iter_idx + 20)
                ]

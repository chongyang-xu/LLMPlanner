...
def get_time_dic_per_response(response, step_num):
    '''
...
def get_time_dic(response_contents, step_num):
    '''
        ...
        if rmsg["step5"] is not None:
            ...
            for i in range(len(sampled_dic['dep_prompts'])):
                ...
                for j in range(len(sampled_dic['dep_prompts'][i])):
                    if (i, j) not in fin_idx_to_loop and (
                            i, 'all') not in fin_idx_to_loop:
                        ...
                        temp_dep_responses.append(
                            dep_results_instances[curr_idx])
                        temp_dep_content.append([
                            choice['message']['content'] for choice in
                            dep_results_instances[curr_idx]['choices']
                        ])
                        edge_list, most_freq_deps, count_freq = get_edge_list([
                            choice['message']['content'] for choice in
                            dep_results_instances[curr_idx]['choices']
                        ])
            ...
            for i in range(len(dep_results_parts)):
                if ((i, 'all') not in nnxt_idx_to_loop) or not check_safety(
                        dep_results_parts[i]):
            ...
            for i in range(len(sampled_dic['dep_prompts'])):
                for j in range(len(sampled_dic['dep_prompts'][i])):
                    if (i, j) in fin_idx_to_loop or (i,
                                                     'all') in fin_idx_to_loop:
                        dep_prompts_instance.append(
                            sampled_dic['dep_prompts'][i][j])
                        res_idx.append((i, j))
                    else:
            ...
            assert len(dep_prompts_instance) == len(
                [kk for k in sampled_dic['dep_prompts'] for kk in k])
            start_idx = 0
            ...
            for i in range(len(sampled_dic['time_prompts'])):
                ...
                for j in range(len(sampled_dic['time_prompts'][i])):
                    if ((i, 'all') not in set(nxt_idx_to_loop) and
                        (i, j) not in set(nxt_idx_to_loop)) or not check_safety(
                            time_results[curr_idx]):
                        ...
                        if sampled_dic['multi_parts'][i]:
                            time_dics, valids = get_time_dic([
                                choice['message']['content']
                                for choice in time_results[curr_idx]['choices']
            ...
            # assert lists are aligned
            assert len(dep_prompts_parts) == len(
                sampled_dic['dep_prompts_part'])
            ...
            for i in range(len(sampled_dic['nec_prompts_instance'])):
                if i not in idx_to_loop:
                    ...
                    curr_idx = curr_idx + len(
                        sampled_dic['nec_prompts_instance'][i])
                    continue
                ...
                for j in range(len(sampled_dic['nec_prompts_instance'][i])):
                    if not check_safety(nec_result_instances[curr_idx]):
                        ...
                        continue
                    nec_responses_instances.append(
                        nec_result_instances[curr_idx])
                    nec_content_instances.append([
                        choice['message']['content']
                        for choice in nec_result_instances[curr_idx]['choices']
                    ])
                    nec_bool_instances.append(
                        veri_nec([
                            choice['message']['content'] for choice in
                            nec_result_instances[curr_idx]['choices']
                        ]))
            ...
            assert len(time_prompts) == len(
                [kk for k in sampled_dic['time_prompts'] for kk in k])
            ...
            rmsg = message["request_message"]
            total_prompt = rmsg["total_prompt"]
            self.nec_parts_results.extend(temp_res)
            if len(self.nec_parts_results) < total_prompt:
                return

            res_dic = {
                'nec_responses_parts': list(),
                'nec_bool_parts': list(),
                'nec_content_parts': list()
            }

            for i, nec_prompt_part in enumerate(nec_prompts_parts):
                if not nec_prompt_part or not check_safety(
                        nec_parts_results[i]):
                ...
                nec_content_parts = [
                    choice['message']['content']
                    for choice in nec_parts_results[i]['choices']
                ]
            ...
            for i in range(len(sampled_dic['nec_prompts_instance'])):
                if i in idx_to_loop:
                    nec_prompts_instances.extend(
                        sampled_dic['nec_prompts_instance'][i])
                else:
                    # add empty prompt to keep list aligned
                    nec_prompts_instances.extend([
                        '' for i in range(
                            len(sampled_dic['nec_prompts_instance'][i]))
                    ])
            # assert lists are aligned
            assert len(nec_prompts_instances) == len(
                [kk for k in sampled_dic['nec_prompts_instance'] for kk in k])
            ...
            start_idx = 0
            self.nec_parts_results = list()
            while start_idx < len(nec_prompts_parts):
                curr_prompts = nec_prompts_parts[
                    start_idx:min(start_idx + 80, len(nec_prompts_parts))]
                """
                temp_res = await generate_from_openai_chat_completion(
                    client,
                    curr_prompts,
                    ...
                    model='gpt-35-turbo')
                msg["total_prompt"] = nec_prompts_parts
                """
                start_idx += 80
...
async def main():
    ...
    parser.add_argument('--api_version', type=str, default=None, required=True)

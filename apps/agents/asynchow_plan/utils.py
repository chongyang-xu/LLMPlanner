...
ERROR_ERRORS_TO_MESSAGES = {
    ...
    APIConnectionError:
        "OpenAI API Connection Error: Error Communicating with OpenAI",  # noqa E501
    Timeout:
...
def check_safety(response: dict):
    ...
    try:
        if response['prompt_filter_results'][0][
                'content_filter_results'] != safe_dic:
            return False
...
async def generate_from_openai_chat_completion(
    ...
    async_responses = [
        _throttled_openai_chat_completion_acreate(client,
                                                  model=model,
                                                  messages=message,
                                                  temperature=temperature,
                                                  top_p=1,
    ...
    all_responses = [
        json.loads(x.model_dump_json(indent=2)) if type(x) is not dict else x
        for x in responses
    ]
...
def get_edge_list(contents):
    ...
    try:
        most_freq_deps, count_freq = Counter([tuple(_) for _ in edge_list
                                             ]).most_common(1)[0]
    except Exception:
...
def get_llama_response_sync(client, task_prompt, model):
    '''
    ...
    for _ in range(3):
        try:
            response = client.text_generation(
                prompt=f'[INST]{task_prompt}[/INST]',
                do_sample=False,
                seed=0,
                max_new_tokens=4096)
            return response
...
def batch_prompt_mistral(model, tokenizer, prompts):
    torch.cuda.manual_seed_all(2024)
    inputs = tokenizer([f'[INST]{prompt}[/INST]' for prompt in prompts],
                       return_tensors="pt",
...
async def benchmark_model(model_name,
    ...
    if 'gpt' in model_name.lower():
        ...
        responses = [
            get_llama_response_sync(client, prompt, model_name)
        ...
        while start_idx < len(prompts):
            responses.extend(
                batch_prompt_mistral(model, tokenizer,
                                     prompts[start_idx:start_idx + batch_size]))
            start_idx += batch_size
    else:
